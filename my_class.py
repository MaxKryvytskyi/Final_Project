...
# new
class Address(Field):  # в розробці 
    
    def __init__(self, street, city=None, country=None):
        self.city = None
        self.country = None
    ...
    def __str__(self):
        return "{}{}{}".format(self.street if self.street else "", ", " + str(self.city) if self.city else "", ", " + str(self.country) if self.country else "")
    

# new
class Email(Field):  # в розробці 
...
class Name(Field):
    ...
    @value.setter
    def value(self, value: str) -> None:
        if len(value) >= 3: self.__value = value
...
class Record:
    ...
    # Додає адрес
    def add_address(self, address): # в розробці new
        self.address = address

    # Додає почту
    def add_email(self, email): # в розробці new
        self.email = email
...
if __name__ == '__main__':
    ...
    try:
        ...
        name = Name("Bill")
    try:
        email = Email("12@gmail.com") # в розробці
    except IncorrectEmailFormat:
        email = Email("max.kriv@gmail.com") # в розробці


    phone = Phone("+38099-333-23-33")
    ad = Address("Героїв небесної сотні 32", "Київ", "Ua") # в розробці
